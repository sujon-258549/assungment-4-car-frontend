...
} from "@/redux/features/auth/Admin/shop";
...
export type CarService = (typeof CAR_SERVICES)[number];
export type PaymentMethod = (typeof PAYMENT_METHODS)[number];
export type Day = (typeof DAYS_OF_WEEK)[number];
const UpdateCarShopForm = () => {
  const [files, setFiles] = useState<File[]>([]);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const { data: shopData, isLoading } = useGetMyShopQuery("shop");
  ...
  useEffect(() => {
    if (shopData) {
      form.reset({
        ...
        carBrands: shopData.carBrands?.map((brand: string) => ({
          value: brand,
        })) || [{ value: "" }],
        servicesOffered: (shopData.servicesOffered || []) as CarService[],
        shopFeatures: shopData.shopFeatures?.map((feature: string) => ({
          value: feature,
        })) || [{ value: "" }],
        socialMediaLinks: {
          facebook: shopData.socialMediaLinks?.facebook || "",
          instagram: shopData.socialMediaLinks?.instagram || "",
          twitter: shopData.socialMediaLinks?.twitter || "",
          linkedin: shopData.socialMediaLinks?.linkedin || "",
        },
        operatingHours: {
          open: shopData.operatingHours?.open || "",
          close: shopData.operatingHours?.close || "",
          daysOpen: (shopData.operatingHours?.daysOpen || []) as Day[],
        },
        paymentMethods: (shopData.paymentMethods || []) as PaymentMethod[],
        customerServiceContact: shopData.customerServiceContact || "",
        serviceAreas: shopData.serviceAreas?.map((area: string) => ({
          value: area,
        })) || [{ value: "" }],
        certifications: shopData.certifications?.map((cert: string) => ({
          value: cert,
        })) || [{ value: "" }],
        warrantyOffered: shopData.warrantyOffered || false,
      });
    }
  }, [shopData, form]);
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      ...
      const res = await updateMutation(formData).unwrap();

      if (res) {
        navigate("/dashboard/details-shop");
        toast.success("Car shop update successfully!", { id: toastId });
        form.reset();
        setFiles([]);
      }
    } catch (error) {
      console.error("Error update car shop:", error);
      toast.error("Failed to update car shop", {
        id: toastId,
      });
    } finally {
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">
            Update Your Car Shop
          </h1>
          <p className="text-gray-600">
            Update in your shop details to get started
          </p>
        </div>
      </div>
...
export default UpdateCarShopForm;
