...
const StudentDashboard = () => {
  ...
  const { data: userData } = useGetSingleUserQuery(user?.email as any);

  return (
    <>
      <div className="flex justify-center items-center py-0 md:py-10">
        <CommonHading text="User Dashboard" color="text-black"></CommonHading>
      </div>
      <div className="profile-card max-w-[600px] mx-auto rounded-md shadow-xl overflow-hidden z-[10] relative cursor-pointer snap-start shrink-0 bg-white flex flex-col items-center justify-center gap-3 transition-all duration-300 group">
        <div className="avatar w-full pt-5 flex items-center justify-center flex-col gap-1">
          <div className="img_container w-full flex items-center justify-center relative z-40 after:absolute after:h-[6px] after:w-full after:bg-cyan-900 after:top-4 after:group-hover:size-[1%] after:delay-300 after:group-hover:delay-0 after:group-hover:transition-all after:group-hover:duration-300 after:transition-all after:duration-300 before:absolute before:h-[6px] before:w-full before:bg-cyan-900 before:bottom-4 before:group-hover:size-[1%] before:delay-300 before:group-hover:delay-0 before:group-hover:transition-all before:group-hover:duration-300 before:transition-all before:duration-300">
            <svg
            ...
            <div className="absolute bg-cyan-900 z-10 size-[60%] w-full group-hover:size-[1%] group-hover:transition-all group-hover:duration-300 transition-all duration-300 delay-700 group-hover:delay-0" />
          </div>
        </div>
        <div className="headings *:text-center *:leading-4">
          ...
          <p className="text-sm font-semibold text-[#434955] mt-2">
            {userData?.email}
          </p>
        </div>
        <div className="w-full items-center justify-center flex"></div>
        <hr className="w-full group-hover:h-5 h-3 bg-cyan-900  group-hover:transition-all group-hover:duration-300 transition-all duration-300" />
      </div>
    </>
  );
};
